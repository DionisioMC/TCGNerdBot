...
class CommandHandlers:
    ...
    async def handle_csv_collection_upload(self, message: discord.Message, attachment) -> None:
        ...
        try:
            ...
            if success:
                ...
                embed.set_footer(
                    text="Your previous collection entries have been replaced with the new data.")
    ...
    async def handle_compare_command(self, message: discord.Message, set_code: str) -> None:
        ...
        try:
            ...
            if comparison:
                embed = create_collection_comparison_embed(
                    set_code, comparison)
                await message.channel.send(embed=embed)
            else:
    ...
    async def handle_help_command(self, message: discord.Message) -> None:
        ...
        try:
            ...
            import asyncio
            asyncio.create_task(self._cleanup_help_navigation(
                help_message.id, 300))  # 5 minutes
    ...
    async def handle_commander_command(self, message: discord.Message, args: List[str]) -> None:
        ...
        try:
            if subcommand == "create":
                success, msg, game = commander_manager.create_game(
                    user_id, channel_id, username)
                if success:
                ...
                if len(args) > 1:
                    ...
                    game_id = args[1]
                    success, msg = commander_manager.join_game(
                    ...
                    available_games = commander_manager.get_joinable_games_for_user(
                        message.channel.id, user_id)
                    ...
                    embed = create_join_games_embed(
                        available_games, message.channel.id)
                ...
                if len(args) > 1:
                    ...
                    game_id = args[1]
                    success, msg = commander_manager.leave_game(
                        game_id, user_id)
                else:
                    # New format: !commander leave (auto-finds your game)
                    success, msg, game = commander_manager.leave_game_by_user(
                        user_id)
                ...
                if success:
                    ...
                    if 'game_id' in locals():
                        ...
                        game = commander_manager.get_user_unfinished_game(
                            user_id)
                ...
                available_placements = commander_manager.get_available_placements(
                    game)
                ...
                success, msg = commander_manager.set_placement(
                    game_id, user_id, placement)
                await message.channel.send(f"{'✅' if success else '❌'} {msg}")
                ...
                game_id = args[1]
                success, msg, game = commander_manager.finish_game(
                    game_id, user_id)
                ...
                archetype_data = edhrec_api.get_commander_archetypes(
                    commander_name)
                ...
                active_games = commander_manager.get_channel_active_games(
                    channel_id)
                ...
                for game in active_games:
                    ...
                    creator_name = next((p['username'] for p in game.players.values(
                    ) if game.creator_id in game.players), "Unknown")
                ...
                meta_stats = analytics.get_server_meta_analysis(
                    channel_id, days)
                ...
                if len(args) > 1 and args[1].lower() == "check":
                    ...
                    # Join remaining args in case name has spaces
                    search_term = " ".join(args[2:])
                    achievement = achievement_manager.find_achievement(
                        search_term)
                    ...
                    player_data = achievement_manager.get_player_achievements(
                        user_id)
                    ...
                    if user_has_earned:
                        ...
                        earned_date = earned_info.get(
                            'earned_at') or earned_info.get('date')

                    # Calculate server statistics
                    total_players = len(
                        achievement_manager.player_achievements)
                    earned_by_count = 0
                    ...
                    embed = create_achievements_overview_embed(
                        user_id, username, player_data, total_achievements)
                    await message.channel.send(embed=embed)

            elif subcommand == "leaderboard":
                # Show achievement leaderboard
                leaderboard = achievement_manager.get_achievement_leaderboard(
                    10)
                ...
                embed.set_footer(
                    text="🎮 Play more games to climb the leaderboard!")
                await message.channel.send(embed=embed)
    ...
    async def handle_placement_reaction(self, reaction: discord.Reaction, user: discord.User) -> None:
        ...
        # Set the placement
        success, msg, game = commander_manager.set_placement_by_reaction(
            user.id, placement)
    ...
    async def handle_join_game_reaction(self, reaction: discord.Reaction, user: discord.User) -> None:
        ...
        # Join the game
        success, msg = commander_manager.join_game(
            selected_game.game_id, user.id, join_data['username'])
    ...
    async def send_commander_help(self, message: discord.Message) -> None:
        ...
        embed.set_footer(
            text="🚀 Enhanced UX • 📊 Analytics • 🏆 Achievements • 2-8 players supported")

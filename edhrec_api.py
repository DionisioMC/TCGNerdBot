...
class EDHRECAPI:
    ...
    def get_commander_archetypes(self, commander_name: str) -> Optional[Dict[str, List[str]]]:
        ...
        try:
            ...
            # PRIORITY 1: Extract Tags from panels.links (most frequent tags ordered by popularity)
            # Note: panels is at the root level, not in container
            panels = commander_data.get("panels", {})
            links = panels.get("links", [])
            
            # Find the Tags section in links
            for link_section in links:
                if link_section.get("header") == "Tags":
                    items = link_section.get("items", [])
                    for item in items:
                        tag_value = item.get("value", "")
                        if tag_value:
                            # Clean up tag names for better archetype names
                            cleaned_tag = self._clean_tag_name(tag_value)
                            if cleaned_tag and cleaned_tag not in archetypes:
                                archetypes.append(cleaned_tag)
                    break
                    
            # Set most popular to first tag if found
            if archetypes:
                most_popular = archetypes[0]
            
            # PRIORITY 2: If no tags found, try themes from container
            if not archetypes:
            ...
            # PRIORITY 3: If no themes found, try to extract from cardlist sections
            if not archetypes:
                container = commander_data.get("container", {})
            ...
            # PRIORITY 4: Provide fallback archetypes if none found
            if not archetypes:
    ...
    def _clean_tag_name(self, tag_name: str) -> str:
        """
        Clean and normalize tag names for better archetype names.
        
        Args:
            tag_name: Raw tag name from EDHREC
            
        Returns:
            Cleaned archetype name
        """
        if not tag_name:
            return ""
        
        # Clean up specific tag patterns
        cleaned = tag_name.strip()
        
        # Convert common tag patterns to more readable archetype names
        tag_mappings = {
            "+1/+1 Counters": "Counters",
            "-1/-1 Counters": "Debuff",
            "Plus 1 Plus 1 Counters": "Counters",
            "Minus 1 Minus 1 Counters": "Debuff",
            "Good Stuff": "Goodstuff",
            "Card Draw": "Draw",
            "Big Mana": "Ramp",
            "Group Hug": "Politics",
            "Group Slug": "Group Hate",
            "Commander Matters": "Voltron",
            "Charge Counters": "Counters",
            "Oil Counters": "Counters",
            "Rad Counters": "Counters",
            "Extra Turns": "Turns",
            "X Spells": "X-Cost",
            "Activated Abilities": "Abilities",
            "Birthing Pod": "Pod",
        ...
        if cleaned in tag_mappings:
            return tag_mappings[cleaned]
